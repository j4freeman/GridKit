...
tables.lines = osm2pgsql.define_way_table('lines', {
    ...
    { column = 'power', type = 'text'},
    { column = 'tags', type = 'jsonb' },
    { column = 'members_nodes', sql_type = 'int8[]'},
    { column = 'members_ways', sql_type = 'int8[]'},
    { column = 'members_relations', sql_type = 'int8[]'},
...
function way_member_nodes(way)
    local ids = {}
    for _, member in ipairs(way.nodes) do
        ids[#ids + 1] = member
    end
    return ids
end

function rel_member_ids(relation)
    local node_ids = {}
    local way_ids = {}
    local relation_ids = {}
    for _, member in ipairs(relation.members) do
        ...
        if member.type == 'r' then
            relation_ids[#relation_ids + 1] = member.ref
        end
    end
...
function osm2pgsql.process_way(object)
    ...
    if object.is_closed and has_area_tags(object.tags) then
        tables.polygons:add_row({
            ...
            power = object.tags.power,
            tags = object.tags,
            members_nodes = '{' .. table.concat(way_member_nodes(object), ',') .. '}',
...
function osm2pgsql.process_relation(object)
    ...
    nodes, ways, relations = rel_member_ids(object)
    ...
    if type == 'route' then
        tables.routes:add_row({
            ...
            power = object.tags.power,
            tags = object.tags,
            member_nodes = '{' .. table.concat(nodes, ',') .. '}',
            member_ways = '{' .. table.concat(ways, ',') .. '}',
            member_relations = '{' .. table.concat(relations, ',') .. '}',
            geom = { create = 'line' }
        })
        return
    end
