...
begin;
drop function if exists merge_power_tags(a jsonb array);
drop table if exists derived_tags;

create function merge_power_tags (a jsonb array) returns jsonb as $$
declare
    r jsonb;
    t jsonb;
    k text;
    v text;
begin
    r = jsonb_build_object('_merged'::text, case when array_length(a, 1) > 1 then 'yes' else 'no' end);
    for t in select unnest(a) loop
        for k in select jsonb_object_keys(t) loop
            if not r ? k then
                r = r || jsonb_build_object(k, t->>k);
